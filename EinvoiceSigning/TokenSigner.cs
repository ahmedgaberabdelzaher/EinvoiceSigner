...
namespace EinvoiceSigning
{


    public class TokenSigner
    {
        public readonly string DllLibPath = "eps2003csp11.dll";
        public string TokenPin = "999999999";
        public string TokenCertificate = "Egypt Trust Sealing CA";

        public void Sign(string folderPath, string pin, string certificateIssuer)
        {
            ...
            {
                Console.WriteLine("First argument " + folderPath);
                if (folderPath.Length >= 2)
                {
                    Console.WriteLine("Second argument " + pin);
                    TokenPin = pin;
                }
                if (pin.Length >= 3)
                {
                    Console.WriteLine("Third argument " + certificateIssuer);
                    TokenCertificate = certificateIssuer;
                }

                if (System.IO.File.Exists(folderPath + @"\SourceDocumentJson.json") == false)
                {
                    Console.WriteLine("The file " + folderPath + @"\SourceDocumentJson.json is not exist");
                }
                else
                {
                    string cades = "";
                    string SourceDocumentJson = System.IO.File.ReadAllText(folderPath + @"\SourceDocumentJson.json");
                    ...
                    System.IO.File.WriteAllBytes(folderPath + @"\CanonicalString.txt", System.Text.Encoding.UTF8.GetBytes(canonicalString));
                    // retrieve cades
                    ...
                    System.IO.File.WriteAllBytes(folderPath + @"\Cades.txt", System.Text.Encoding.UTF8.GetBytes(cades));
                    ...
                    System.IO.File.WriteAllBytes(folderPath + @"\FullSignedDocument.json", System.Text.Encoding.UTF8.GetBytes(fullSignedDocument));
                }
            }
        }
